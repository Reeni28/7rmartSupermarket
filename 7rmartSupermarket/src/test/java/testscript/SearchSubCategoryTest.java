...
public class SearchSubCategoryTest extends Base {
	...
	public void searchsubcategory() throws IOException {
		...
		searchsubcategorypage.subcatclick().subcatsearch().categorydropdownlist(catvalue).subcategory(subvalue)
				.search();
		boolean alertmsgisloaded = searchsubcategorypage.islistisloaded();
		Assert.assertTrue(alertmsgisloaded, Constant.ERRORMESSAGEFORSUBCATSEARCH);
	}
}
